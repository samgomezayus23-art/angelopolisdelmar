...
async function main() {
  try {
    ...
    process.stdout.write(`Using URL: ${supabaseUrl}\n`);
    ...
    for (const feed of feeds) {
      process.stdout.write(`Syncing ${feed.platform} for ${feed.property_id}...\n`);
      const { error } = await supabase
        .from('ical_feeds')
        .upsert(feed, { onConflict: 'property_id,platform' });
      
      if (error) process.stdout.write(`Error: ${JSON.stringify(error)}\n`);
      else process.stdout.write(`Success for ${feed.property_id}\n`);
    }
